...
export class Responsive2D extends Component {
    ...
    resize() {
        ...
        // if (node.name === 'Button') {
        //     let eff1 = tween()
        //         .by(0.5, { scale: new Vec3(-0.05, -0.05, -0.05) }) // Thu nhỏ
        //         .by(0.3, { scale: new Vec3(0.05, 0.05, 0.05) }) // Phóng to
        //     if (DataManager.instance.eff)
        //         DataManager.instance.eff.stop();
        //     DataManager.instance.eff = tween(node).repeatForever(eff1).start();// Bắt đầu tween
        // }
        ...
        if (alignment) {
            ...
            if (verticalSpaceType == 1) {
                // custom responsive vertical
                let ratio = width / height;
                if (width < height) {
                    switch (this.node.name) {
                        // case 'Title':
                        //     ratio > 0.67 ? this.node.active = false : this.node.active = true;
                        //     break;
                        // case 'Button':
                        //     // this.node.scale = new Vec3(0.3, 0.3, 0);
                        //     ratio > 0.67 ? verSpace = 0 : 0
                        //     break;
                        default:
                            break;
                    }
